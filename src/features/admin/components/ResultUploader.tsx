...
export const ResultUploader: React.FC<ResultUploaderProps> = ({ isOpen, onClose, tournament, group, match, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Update participant stats — index participants by userId, teamId, and doc ID
            // to support both solo and team tournament structures.
            ...
            // Map teamId/userId/docId -> list of participant records
            const participantsMap = new Map<string, { ref: any; data: any }[]>();
            for (const docSnap of participantsSnap.docs) {
                const pData = docSnap.data();
                const record = { ref: docSnap.ref, data: pData };
                ...
                for (const key of keys) {
                    ...
                    participantsMap.get(key)!.push(record);
                }
            }

            const updatedRefs = new Set<string>();
            for (const res of scoredResults) {
                const matchingParticipants = participantsMap.get(res.teamId) || [];
                for (const p of matchingParticipants) {
                    if (updatedRefs.has(p.ref.id)) continue;
                    updatedRefs.add(p.ref.id);
                    batch.update(p.ref, {
