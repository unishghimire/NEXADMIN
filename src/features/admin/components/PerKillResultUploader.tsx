...
export const PerKillResultUploader: React.FC<PerKillResultUploaderProps> = ({
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Filter out any previous entries for this specific match to avoid duplicates on re-upload/edits
            const existingRewards = ((tournament as any).killRewards || []) as PlayerKillReward[];
            const filteredRewards = existingRewards.filter(r => r.matchId !== match.id);
            const mergedRewards = [...filteredRewards, ...killRewards];

            // Save to tournament document — update killRewards with merged list
            const tournamentRef = doc(db, 'tournaments', tournament.id);
            await updateDoc(tournamentRef, {
                killRewards: mergedRewards,
